...
const DEFAULT_MAP_ZOOM = 1.68;
const MAX_MAP_ZOOM = 2.78;
const MAP_ZOOM_STEP = 0.28;
const EVACUATION_TUNNEL = "/manus-storage/rescue-evacuation-tunnel_41ac4062.png";
const ISLAND_CITY_ART = "/manus-storage/rescue-network-expanded-map-port-school-road_3b8a5bf1.webp";
const AMBULANCE_PREMIUM = "/manus-storage/rescue-ambulance-premium_5a2a0c48.png";
const LIGHTNING_ARTS = ["/manus-storage/rescue-lightning-calinou-alpha_1f6ef807.webp"] as const;
const TARGET_ASSETS: Partial<Record<NodeId, string>> = {
  residential: "/manus-storage/rescue-residential-premium_561e2840.png",
  hospital: "/manus-storage/medical-target-sprite_ba5114ad.png",
  school: "/manus-storage/school-target-sprite_8c3a4305.png",
  harbor: "/manus-storage/harbor-target-sprite_4ed2944c.png",
  radio: "/manus-storage/rescue-radio-premium_dcd7d237.png",
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const preloadAudio = (url: string) => new Promise<void>((resolve, reject) => {
      ...
      audio.onerror = () => reject(new Error(url));
      audio.src = url;
      audio.load();
    });
    ...
    <main className={`game-shell landscape-shell ${isLandscape ? "is-landscape" : "is-portrait"}`} dir="rtl" data-phase={game.phase} data-storm-stage={stormStage} data-storm-volume={getStormVolumeForStage(stormStage)} data-thunder-interval={getThunderIntervalForStage(stormStage)} data-forced-lightning={forcedLightningRef.current ? "true" : undefined} data-forced-radio-strike={forcedRadioStrikeRef.current ? "true" : undefined} data-real-audio="cc0">
      ...
      <section className="game-frame" aria-label="لعبة شبكة الإنقاذ">
        ...
        <section className="playfield">
          <svg className={`rescue-map ${stormActive ? "storm-active" : ""} ${weatherActive ? "weather-active" : ""} ${game.phase === "finished" ? "storm-landfall" : ""} ${finaleActive ? "mission-complete" : ""}`} viewBox={`${mapView.x} ${mapView.y} ${mapViewSize.width} ${mapViewSize.height}`} data-map-zoom={mapZoom.toFixed(2)} data-map-default-zoom={DEFAULT_MAP_ZOOM} data-map-view={`${mapView.x},${mapView.y}`} role="img" aria-label="خريطة تفاعلية قابلة للتكبير لمدينة ساحلية أثناء عاصفة" onPointerDown={handleMapPointerDown} onPointerMove={handleMapPointerMove} onPointerUp={finishMapPointer} onPointerCancel={(event) => { pointersRef.current.delete(event.pointerId); drawingRef.current = false; setDrawing(false); panRef.current.active = false; pinchRef.current.active = false; }}>
            <defs>
